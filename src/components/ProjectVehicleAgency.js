import React from 'react';

function ProjectVehicleAgency() {
  return (
    <section className="project-section" style={{ textAlign: 'center', maxWidth: 700, margin: '40px auto', background: '#fff', borderRadius: 12, boxShadow: '0 2px 12px rgba(0,0,0,0.07)', padding: 32 }}>
      <img
        src="https://images.unsplash.com/photo-1503736334956-4c8f8e92946d?auto=format&fit=crop&w=700&q=80"
        alt="Car dealership"
        style={{ width: '60%', maxWidth: 340, borderRadius: 10, marginBottom: 24, boxShadow: '0 1px 8px rgba(0,0,0,0.08)' }}
      />
      <h2 style={{ fontWeight: 800, color: '#1a237e', marginBottom: 18 }}>Vehicle Agency</h2>
      <p style={{ fontSize: '1.1rem', color: '#222', marginBottom: 18 }}>
        Vehicle Agency is an application for managing a car dealership, allowing you to manage vehicle inventory, customers, deals, and track vehicle status. The system provides a convenient management interface, options to add and update vehicles, manage sales and rental deals, and advanced search tools to find the right car.
      </p>
      <ul style={{ display: 'inline-block', textAlign: 'left', fontSize: '1.05rem', color: '#222', margin: '0 auto', padding: 0, listStyle: 'disc inside', lineHeight: 1.7 }}>
        <li>Manage vehicle inventory, customers, and deals</li>
        <li>Advanced search and filtering tools</li>
        <li>Intuitive management interface</li>
        <li>Developed in Java</li>
      </ul>
    </section>
  );
}

export default ProjectVehicleAgency; 